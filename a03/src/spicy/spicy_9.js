...
export const everyEven = (arr, test) => {
    ...
    for (let i = 0; i < arr.length; i+=2) {
        evenResult = evenResult && test(arr[i]);
    }
    return evenResult;
};
...
export const filter = (arr, test) => {
    ...
    arr.forEach(token => {
        if (test(token)) {
            passArray.push(token)
        } else {
            failArray.push(token)
        }
    });
        let obj = {
            pass: passArray,
...
export const allEvensAreOdd = (arr) => {
    return everyEven(arr, even => even % 2 == 1);
};
...
export const anEvenIsOdd = (arr) => {
    return someEven(arr, even => even % 2 == 1);
};
